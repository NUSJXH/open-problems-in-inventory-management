import { colors, selectYears, makeSeries, upperBound } from './chart-model.mjs?v=3.0.0';
...
async function update(){
  ...
  try{
    if(!dataCache.has(source)){
      ...
      const response=await fetch(`data/${source==='wos'?'wos':'openalex'}-trends.json?v=3.0.0`);
...
update();
fetch('data/acquisition-summary.json?v=3.0.0').then(r=>{if(!r.ok)throw new Error(r.status);return r.json();}).then(data=>{
